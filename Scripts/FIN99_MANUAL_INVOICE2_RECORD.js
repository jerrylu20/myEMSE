/*------------------------------------------------------------------------------------------------------/
| Program: FIN99_Manual_Invoice2_Record.js  Client : Oakland   
|1. Script forces lvl 1 invoices with balance to lvl 2 regardless of invoice age
|2. 
| To install Script:
...
var emailAddress = ("Kzhu@oaklandnet.com","DRex@oaklandnet.com","AGPlacido@oaklandnet.com");//("DRex@oaklandnet.com");	// email to send report
var emailText = "";
...
var model = aa.set.getSetDetailsScriptModel().getOutput();
model.setSetID(setCode);
var SetMemberArrayObj=aa.set.getSetMembers(model);

if (SetMemberArrayObj.getSuccess()){
	SetMemberArray = SetMemberArrayObj.getOutput().toArray();
}
...
for (var i = 0; i < SetMemberArray.length; i++)
{
	...
	if(!printInvoice){
		logDebug("No Level 2 Invoices Found for : " + altID);
		continue;
	}
