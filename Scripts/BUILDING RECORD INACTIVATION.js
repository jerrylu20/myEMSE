...
vStatusSkip.push('Permit Expired');
vStatusSkip.push('COMPLETED');
vStatusSkip.push('Completed Cert Received');
vStatusSkip.push('NOTICE EXPIRED');
vStatusSkip.push('CERT. OF COMPLETION');
...
getCapIdResult = aa.cap.getCapIDsByAppSpecificInfoDateRange('KEY DATES', 'Permit Inactivation Date', vBeginDate, vEndDate); 
...
for (i in capIdArray) { 
	...
	if (!exists(capStatus,vStatusSkip) && appTypeArray[0] == 'Building') {
		aa.print("Updating record: " + capIDString + ". Setting record status to 'Permit Inactive'.")
		updateAppStatus('Permit Inactive','Permit Updated by batch script on ' + vTodayString, capId);
		vUpdated++;
	}
}
