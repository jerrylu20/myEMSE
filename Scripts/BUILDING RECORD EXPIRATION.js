...
getCapIdResult = aa.cap.getCapIDsByAppSpecificInfoDateRange('KEY DATES', 'Permit Expiration Date', vBeginDate, vEndDate); 
...
for (i in capIdArray) { 
	...
	if (!exists(capStatus,vStatusSkip) && appTypeArray[0] == 'Building') {
		aa.print("Updating record: " + capIDString + ". Setting record status to 'Permit Expired'.")
		updateAppStatus('Permit Expired','Permit Updated by batch script on ' + vTodayString, capId);
		vUpdated++;
	}
}
