/*------------------------------------------------------------------------------------------------------/
| Program: FIN03_LienProcessing.js  Client : Oakland   
...
| 5/13/14 - dq - CHANGED add fee of PL_PREP_INV to PL_PREP_INV1 
| 5/14/14 - dq - CHANDED Recordation date to not be setup when lien created
| 5/20/14 - dq - CHANGED to only process invoices to lien one time.  Stores lien in udf1 of f4invoice field for tracking
| 5/21/14 - dq - CHANGED to ignore 0 balance due invoices when generating lien
| 8/15/14 - dq - ADDED code to move record owner to lien Lienee contact for reporting
-------------------------------------------------------------------------------------------------------*/
/*------------------------------------------------------------------------------------------------------/
| BEGIN Includes
/------------------------------------------------------------------------------------------------------*/
...
if (setPrefix != "")
{
	...
	mySet = new capSet(setName,setName,"FINLienRecs");
	mySet.status = "Not Processed";
	mySet.update();
	//Correctly set the set status
	var setId = aa.set.getSetByPK(setName).getOutput();
	setId.setRecordSetType("FINLienRecs");
	setId.setSetStatus("Not Processed");
	aa.set.updateSetHeader(setId);
}
...
for (var i = 0; i < SetMemberArray.length; i++)
{
	...
	if(createNew){
		...
		copyOwnerToLienee(capId,newRecID); //copy owner to lienee contact
	...
	for (thisInv in invList)  // for each Invoice
		...
		//If already processed the ignore this invoice
		if (thisInvoice.getInvoiceModel().getUdf1() != null)
			continue;
			
		//IGNORE 0 BALANCE INVOICES
		if (thisInvoice.getInvoiceModel().getBalanceDue() == 0)
			continue;
		...
		if (invoiceLvl == "2")
			...
			updateInvoiceLevel(capId,thisInvoice,2,null,null,newRecAltID);
	...
	if(createNew){  //only add the fees to the lein once
		...
		addFee("PL_PREP_INV1","PRIO_LIEN","FINAL",1,"N",newRecID);
		...
		for (thisLienInv in invLienList)  // for each Invoice
			...
			if (invoiceLienLvl == "1")
				{
					updateInvoiceLevel(newRecID,thisLienInvoice,2,datePlus30,today,newRecAltID);
				}
			}	
	} //end create new
...
function copyOwnerToLienee(recId,newRecId){
	var owners = aa.owner.getOwnerByCapId(recId).getOutput();
	if(owners != null){
		owner = owners[0];
		var vLienee = aa.people.createPeopleModel().getOutput();
		vLienee.setContactType("Lienee");
		vLienee.getCompactAddress().setAddressLine1(owner.getMailAddress1());
		vLienee.getCompactAddress().setAddressLine2(owner.getMailAddress2());
		vLienee.getCompactAddress().setState(owner.getMailState());
		vLienee.getCompactAddress().setCity(owner.getMailCity());
		vLienee.getCompactAddress().setZip(owner.getMailZip());
		vLienee.setFullName(owner.getOwnerFullName());
		vLienee.setPhone1(owner.getPhone());
		vLienee.setFax(owner.getFax());
		aa.people.createCapContactWithRefPeopleModel(newRecId, vLienee.getPeopleModel());		
	}
}