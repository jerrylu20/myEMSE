...
function payByCapFeeInvObj(feesObj,methodType) //optional: itemCap
  {
  	for(altId in feesObj){
		...
		if(amtToPay > 0){
			...
			//MAKE THE PAYMENT
                        //DCB Change to use function for 8.0
			//presult = aa.finance.makePayment(p);
			presult = makePaymentProxy(p);
			var pSeq = null;
			var pR = null;
...
function payInvoiceByMethod(invoiceNbr,methodType) //optional: itemCap
  {
	...
	if(amtToPay > 0){
		...
		//MAKE THE PAYMENT
		//DCB Change to function for 8.0 fix
		//presult = aa.finance.makePayment(p);
		presult = makePaymentProxy(p);
		var pSeq = null;
		var pR = null;
...
//DCB Fixes issue from 8.0 
function makePaymentProxy(payModel){ 
	var payNum = null; 
	var result; 
