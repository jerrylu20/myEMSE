/*------------------------------------------------------------------------------------------------------/
| Program: FIN00_OAKLANDLIEN_SETGEN V1.0.js  Trigger: Batch    
| Client: Oakland   Request # SR5059
| 
| Version 1.0 - Base Version. 3/1/13 KG
| 1.1 - 5/28/14 DQ - Updated to create lien sets  

aa.env.setValue("appGroup","Enforcement");				//   app Group to process {Licenses}
aa.env.setValue("appTypeType","*");			//   app type to process {Rental License}
aa.env.setValue("appSubtype","*");			//   app subtype to process {NA}
aa.env.setValue("appCategory","*");			//   app category to process {NA}
aa.env.setValue("setName","ENFORCEMENT_LIEN");				//   Prefix for set ID
aa.env.setValue("agingDate","");				// Aging Date
aa.env.setValue("emailAddress","dquatacker@accela.com");			// email to send report
...
if (agingDate == ""){
	...
	agingDate = jsDateToMMDDYYYY(agingDateObj);
}
else
...
function mainProcess()
	...
	var invAgeFilter = 0;
	var invoiceLevel2 = 0;
	var invoiceCredtDue = 0;
	var invoiceAlreadyLiened = 0;
	...
	for (thisInv in invList)  // for each Invoice
	{
		...
		//Filter non-lvl2 invoices
		if(invoiceLvl != "2"){
			invFilter++;
			continue; 
		}
		var invoiceBalDue = thisInvoice.getInvoiceModel().getBalanceDue();
		if(invoiceBalDue < 0){
			invoiceCredtDue++;
			continue;
		}
		...
		if(thisInvoice.getInvoiceModel().getUdf1() != null){
			invoiceAlreadyLiened++;
			continue;
		}
		
		// Calculate age
		// Ignore non invoiced batch lvl 2
		if(thisInvoice.getInvoiceModel().getInvBatchDate() == null){
			invAgeFilter++;
			continue;
		}
		invoicePrintDt = thisInvoice.getInvoiceModel().getInvBatchDate();	
		var invoiceDate = convertDate(aa.date.parseDate(invoicePrintDt));
		invoiceAge = daysBetween(invoiceDate,agingDateObj);
		
		// filter on invoices less than 30 days old
		if(invoiceAge < 30){
			invAgeFilter++
			continue;
		}
	...
	aa.sendMail("noreply@accela.com", emailAddress, "", batchJobName + " Job Statistics" ,  "Total unpaid invoices : " + invList.length + "<BR>" + 
 												"Total invoices ignored due to level : " + invFilter + "<BR>" +
 												"Total invoices ignored already liened : " + invoiceAlreadyLiened + "<BR>" +
 												"Total invoices ignored due to age : " + invAgeFilter + "<BR>" +
												"Total invoices ignored due to application type filter : " + capFilterType + "<BR>" + 
 												"Total invoices examined : " + capCount + "<BR>" +  
												"Invoice Level 2 Found to Lien : " + invoiceLevel2 + "<BR>");
	
 	logMessage("INFO","Total unpaid invoices: " + invList.length);
 	logMessage("INFO","Total invoices ignored due to level : " + invFilter );
 	logMessage("INFO","Total invoices ignored due to credit : " + invoiceCredtDue);
 	logMessage("INFO","Total invoices ignored already liened : " + invoiceAlreadyLiened);
 	logMessage("INFO","Total invoices ignored due to age : " + invAgeFilter );
 	logMessage("INFO","Ignored due to application type: " + capFilterType);
 	logMessage("INFO","Total CAPS processed: " + capCount);
 	logMessage("INFO","Invoice Level 2 Found to Lien : " + invoiceLevel2);
...
function updateInvoiceLevel(c,iObj,newLevel,dueDate,batchDate)
	{
	var i = iObj.getInvoiceModel()
	editResult = aa.finance.editInvoice(c,
					i.getInvNbr(),
