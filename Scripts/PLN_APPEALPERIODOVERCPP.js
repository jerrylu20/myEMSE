/*------------------------------------------------------------------------------------------------------/
| Oakland Planning Appeal Period Over CPP Batch
|
| 06/20/2013 - KG
/------------------------------------------------------------------------------------------------------*/
			...
			*/
			...
			if(appealEndDate != "undefined" && appealEndDate != null && appealEndDate != "")
			{
				...
				if (todayJS > appealEndDateJS)
				{
					...
					{
						updateTask("Closure","Approved","Updated by Batch Script","");
						logDebug("Workflow Task Closure was updated to Approved on Record " + capIDshow);
						capWorkflowUpdated++;
					}
					
					if(capStatus == "Denied-Pending Appeal")
					{
						updateTask("Closure","Denied","Updated by Batch Script","");
						logDebug("Workflow Task Closure was updated to Denied Modified on Record " + capIDshow);
						capWorkflowUpdated++;
					}
